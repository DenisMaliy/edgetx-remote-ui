...
/* --- запобіжник: коли чекати вже немає сенсу ------------------------------
 *
 * Рішення людини за критерієм 4.1 задачі 0019. Без нього на безперервному
 * гортанні показ падає до ~4 кадр/с — і кадри однаково лишаються зшитими,
 * бо карта не порожніє. Тобто вийшло б строго гірше, ніж було.
 *
 * Поріг відповідає на питання: **чи встигне залишок доїхати раніше, ніж
 * пульт перемалює кадр наново?** Якщо ні — чекати нема сенсу: те, чого ми
 * чекаємо, застаріє швидше, ніж прийде, і правильна дія — показати як є.
 *
 * ⚠️ Виведення, і в ньому одна тонкість, через яку ці рядки й написані.
 *
 * Плитка на дроті ≈ 286 Б; дріт на 2 625 000 бод дає 262 500 Б/с, тобто
 * **1.1 мс на плитку**. Кадр пульта — 50 мс. Отже:
 *
 *     поріг = 50 мс / 1.1 мс ≈ 45 плиток
 *
 * Тонкість: строк очікування вище рахується по **3 мс** на плитку, а не по
 * 1.1 — і це не суперечність, обидва числа правильні. Різниць насправді дві,
 * і обидві навмисні:
 *
 *   строк:  400 Б (найгірша заміряна плитка) / 123 000 Б/с (Wi-Fi) = 3.25 мс
 *   поріг:  286 Б (середня плитка)           / 262 500 Б/с (дріт)  = 1.09 мс
 *
 * 1. **Дріт, а не наскрізний шлях.** Поріг **порівнює дві величини**: коли
 *    доїде залишок і коли доїде наступний кадр. Конвеєр (міст, Wi-Fi, браузер)
 *    затримує обидва однаково й у порівнянні скорочується.
 * 2. **Середня плитка, а не найгірша.** Поріг описує типовий стан і має
 *    вмикатися не надто рано; строк описує ризик, і ціна помилки в ньому
 *    несиметрична, тому там береться найгірше з баченого.
 *
 * Не «виправляйте» одне число під інше — вийде поріг 32 замість 45.
 *
 * Для TX16S 45 збігається з третиною сітки (135 / 3), але це збіг: поріг
 * виведено з часу, а не з розміру сітки, і на іншому пульті збігу не буде.
 */

// ⚠️ Заміряно на TX16S: середня плитка 32×32 RGB565 після RLE, повний кадр
// задачі 0018. На ЧБ-пульті (етап 5.3) плитка буде в рази менша, поріг
// відповідно занизиться — тоді це число доведеться брати з HELLO або
// переміряти, а не лишати як є.
const FRAME_WAIT_TILE_BYTES = 286;

// Кадр пульта. ⚠️ Число обрала людина, і поруч є два сусідні, які спокушають
// «виправити» його — не робіть цього без нового заміру:
//   - LVGL перемальовує раз на 30 мс (`lv_conf.h`, LV_DISP_DEF_REFR_PERIOD),
//     але це період **перемальовки**, а не період, з яким закривається наш
//     FRAME_END;
//   - заміряна частота FRAME_END у задачі 0018 — 17.1/с, тобто 58 мс.
// 50 мс — округлення заміряних 58 **вниз**, тобто в бік меншого порога:
// помилитись у бік «чекав дарма» дорожче, ніж у бік «показав одразу».
// 30 мс дали б поріг 27, 58 мс — 53.
const RADIO_FRAME_MS = 50;

// Запасне значення, коли швидкість каналу не застосовна (TCP у симуляторі,
// USB CDC). ⚠️ Це саме запасний варіант, а не стеля: додайте в перелік
// швидкість вищу за 2 625 000 — і обчислений поріг буде більший за це число.
const FRAME_WAIT_TILE_LIMIT_FALLBACK = 45;

/**
 * Скільки плиток ще має сенс чекати.
 *
 * ⚠️ Рахується зі **швидкості каналу**, а не береться сталою, бо швидкість у
 * цьому проєкті міняється на ходу (задача 0017). Поріг, прибитий до
 * 2 625 000, на 921600 мовчки завищив би очікування втричі — клієнт чекав би
 * того, що за побудовою не встигне.
 *
 * @param {number|null} baud поточна швидкість каналу з HELLO; null — коли
 *   поняття не застосовне (TCP у симуляторі, USB CDC). Тоді запасне значення.
 */
function frameWaitTileLimit(baud) {
  if (!baud) return FRAME_WAIT_TILE_LIMIT_FALLBACK;
  const bytesPerFrame = (baud / 10) * (RADIO_FRAME_MS / 1000);
  // Округлення **вниз**: питання буквально «скільки цілих плиток устигає
  // вміститися в кадр пульта». Половина плитки не доїде.
  return Math.max(1, Math.floor(bytesPerFrame / FRAME_WAIT_TILE_BYTES));
}
...
const RemoteUI = {
  ...
  FRAME_WAIT_TILE_BYTES, RADIO_FRAME_MS, FRAME_WAIT_TILE_LIMIT_FALLBACK, frameWaitTileLimit,
  parseFrameEnd,
  HELLO_MIN,
