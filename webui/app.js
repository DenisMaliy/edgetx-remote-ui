...
function fitCanvas() {
  ...
  if (portrait()) {
    ...
    // ⚠️ Доступна висота рахується від **самих панелей**, а не від ряду
    // зображення. `wrap.clientHeight` тут більше не годиться: нижче ми самі
    // піднімаємо панелі, і ряд зображення через це росте — читати його
    // означало б читати власний попередній результат.
    //
    // Береться `offsetHeight` панелі, і в ньому немає нашого `margin-bottom`,
    // тобто число лишається природною висотою панелі й від картинки не
    // залежить. Тому обчислення сходиться за один прохід, без коливань.
    ...
    // ⚠️ `want` рахується від висоти **сцени**, `room` — від того, що лишили
    // панелі. Два різні числа: перше каже, де центр вікна, друге — скільки
    // місця є під картинкою. Панель тіснить зображення рівно тоді, коли друге
    // менше за перше.
    const want = Math.round((stage.clientHeight - ih) / 2);
    const room = availH - ih;
    const top = Math.max(0, Math.min(want, room));
    setStyle(canvas, 'marginTop', top + 'px');

    // ⚠️ Критерій 9.1: панелі прив'язані до нижнього краю **зображення**, а не
    // до нижнього краю вікна. Ряд панелей стоїть у сітці останнім, тобто
    // прибитий до дна, — і весь надлишок місця опинявся між картинкою й
    // ручками згортання. Тепер той надлишок віддається панелям як відступ
    // знизу: ряд опускається рівно настільки, наскільки картинка не дійшла до
    // низу, а ручки лишаються приліпленими до неї в кожному стані.
    //
    // ⚠️ Відступ ставиться всім трьом учасникам ряду: висоту ряду задає
    // найвищий, і відсунути лише панелі означало б лишити смужку висіти.
    const lift = Math.max(0, room - top) + 'px';
    for (const el of [padL, padR, bar]) {
      if (el) setStyle(el, 'marginBottom', lift);
    }
    return;
  }

  // Альбомна зсуває картинку по горизонталі, тож вертикальні зсуви книжкової
  // треба зняти: телефон повертають у руках, і стилі лишаються від попередньої
  // орієнтації. Разом із зсувом картинки — і відступ під панелями: в альбомній
  // смужка стану займає власний ряд, і зайві пікселі під панелями відсунули б
  // її від низу вікна.
  setStyle(canvas, 'marginTop', '');
  for (const el of [padL, padR, bar]) {
    if (el) setStyle(el, 'marginBottom', '');
  }
