...
**Вердикт: прийнято.** 2026-07-26. Кроки 2.3, 2.4 і 2.5 закрито.
**Цілі MVP №1 і №2 досягнуто в дротовому варіанті.**

### Перевірено власними командами

| Твердження | Доказ |
|---|---|
| Задача не додала коду в EdgeTX | `git -C upstream/edgetx diff --numstat` → **88 вставок, 0 вилучень**, тобто рівно те саме, що після 0009. Прошивання коду не змінює — так і має бути |
| Дерево чисте | лише `.claude/settings.json`, як домовлено |
| Коміти розділені осмислено | кроки 0–2, кроки 3–4, закриття — окремо. По них видно хід роботи, а не купу |

### Знімки — доказ, а не ілюстрація

Подивився всі чотири, і два з них варті окремої згадки.

**`04-about-our-build.png`** — екран «Про EdgeTX», знятий **через наш власний
транспорт**: `VERS: pre-2.12.2-selfbuild (3673c518)`, дата 2026-07-26. Тобто
на картинці наша збірка сама себе й називає. Ланцюг замкнувся: ми бачимо на
комп'ютері екран пульта, який показує, що в ньому наша прошивка.

**`03-debug-mixer-under-load.png`** — `Tmix max 0.46 ms`, `Period: 4 ms`.
Головна перевірка безпеки з ТЗ, і вона не просто пройдена — вона **знята через
той самий канал, безпеку якого доводить**. Красиво.

Спосіб замірювання теж правильний: скинути лічильник, наситити канал,
перечитати, **і окремо переконатися, що скидання спрацювало**. Останнє —
різниця між заміром і самообманом.

### Причина підлагування названа правильно

Розрізнення зроблено так, як просила задача — заміром, а не відчуттям.
95 мс на клацання енкодера проти 1149 мс на повний екран, різниця рівно в
байтах. Розкид між прогонами ±0.4% додає останній цвях: процес, обмежений
процесором, так не поводиться, у нього розкид більший і залежить від того, що
ще крутиться.

Отже вузьке місце — **дріт**, як і передбачало вимірювання в задачі 0006
(230 КіБ/с потреби проти ~90 наявних). Це важливо для наступного кроку:
оптимізувати захоплення чи стиснення зараз було б витраченою роботою.

### Аналізатор спектра замість ELRS — заміна вдала

ELRS справді неможливий на цьому залізі: внутрішній модуль TX16S — MULTI, і
ELRS серед його протоколів немає за побудовою. Це не відмовка, це факт про
залізо.

А заміна вийшла **сильнішою за початковий критерій**. ELRS-конфігуратор мав
довести, що ввід доходить до Lua-шару. Аналізатор спектра на 90.4% каналу
доводить більше: екран, який бруднить плитки сам і безперервно, тобто
найважчий можливий випадок для транспорту. Саме той, заради якого в 0009
з'явилася третя причина закриття кадру.

### Три помилки, названі самою командою

Те, що вони названі, вартує окремої подяки — жодну з них я б не побачив за
результатом.

**Головна — зсунутий робочий каталог.** Команда журналу зібралася з
неіснуючих шляхів і впала на `realpath`. Пощастило: це був **крок за один хід
до `dfu-util -D`**. Якби впало на хід пізніше, у флеш пішло б невідомо що, і
розбиралися б ми з пультом, який не вмикається.

Висновок «далі всі шляхи абсолютні» правильний, але замалий. Записую ширше
правило в журнал рішень: **у процедурах, які пишуть у флеш, відносних шляхів
не існує**. Не тому, що зручніше, а тому, що `cd` — це стан, а стан у
незворотних операціях є вадою.

Дві інші — кінцевий пробіл у файлі, який правлять наосліп, і безглуздий
перший замір із головного екрана — теж названі чесно й теж варті запису.

### ⚠️ Одне питання за доказами

На знімку `03-debug-mixer-under-load.png` є **чорний прямокутник** приблизно
на місці `Duration(ms)`. На решті трьох знімків артефактів немає.

Це може бути одне з двох, і різниця істотна:

- **нормально:** плитка не влізла в канал, лишилась брудною і пішла наступним
  кадром — саме та плавна деградація, яку ми ухвалили. Знімок просто спіймав
  момент;
- **вада:** плитка загубилась назовсім і чорнота лишається. Тіньова копія в
  тому місці ніколи не заповнювалась — а це та сама порода помилки, яку
  рецензія 0009 уже одного разу ловила (`captureMarkAllDirty()` до першого
  флешу віддавав неініціалізовану SDRAM).

Розрізнити просто: подивитись на живий екран і сказати, чи чорнота зникає.
**Не блокує приймання** — задача про прошивання, а не про захоплення, — але
відповідь потрібна до кроку 2.6, бо на мості канал буде ще вужчим.

### Дві знахідки для асистента

**`keyShortcuts` не читаються на 2.x** — беру в роботу: це стосується правок
`radio.yml` наосліп, тобто мого документа про залізо. Допишу застереження,
щоб хтось не витратив вечір на налаштування, якого прошивка не прочитає.

**`tools/capture_check.py` лишився на TCP** — дрібниця, йде повз міст,
полагодьте принагідно.

**Підсумок.** Пульт із розбитим екраном показує свій екран на комп'ютері й
слухається. Мікшер не зрушив під навантаженням у 88.5% каналу. Причина
підлагування названа заміром, а не здогадом. Три власні помилки названі
вголос, включно з тією, що ледь не коштувала прошивки. Приймаю.
