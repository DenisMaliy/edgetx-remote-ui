...
**Чим перевірено.** Репозиторій — до читання звіту. `git log --oneline` і
`git show --stat` по обох комітах; **прочитано весь `hooks.patch` рядок за
рядком** і звірено з переліком у `docs/05-hooks.md`;
`diff <(git -C upstream/edgetx diff) hooks.patch`; **власний прогін
`patch-revert.sh` → перевірка чистоти дерева → `patch-apply.sh` → повторна
звірка з патчем**; перевірка, чи лишився symlink і рядок у `.git/info/exclude`
після зняття; **власна збірка й прогін усіх тестів** командою з README;
перегляд самого PNG.

| Критерій | Вердикт | Доказ, який бачив рецензент |
|---|---|---|
| Два коміти, не змішані | ✅ | `498fcfb` — тільки нові файли; `2b5068e` — патч і скрипти. Жодного файлу EdgeTX у першому |
| Чужого коду мінімум | ✅ | `hooks.patch` — **14 доданих рядків**, два файли, нуль видалених |
| Тільки дозволені файли | ✅ | `gui/colorlcd/lcd.cpp` = пункт 5, `radio/src/CMakeLists.txt` = пункт 8. Виходу за перелік немає |
| Усе під прапорцем | ✅ | у `lcd.cpp` обидві вставки в `#if defined(REMOTE_UI)`; у CMake — `option(... OFF)` і `if(REMOTE_UI)` |
| Гачок у правильному місці | ✅ | вставка стоїть **до** блоку `#if !defined(LCD_VERTICAL_INVERT)`, як велить документ |
| Механізм накладання | ✅ | **прогнав сам**: `revert` → `git status --short` порожній, symlink зник, рядок з `info/exclude` прибрано; `apply` → дерево побайтово збігається з патчем |
| Дерево = патч, без «хвостів» | ✅ | `diff` між `git diff` у upstream і файлом патчу — порожній |
| Тести | ✅ | **зібрав і прогнав сам**: 42 з 42, 0.032 с, під `-fsanitize=address,undefined` |
| Картинка як доказ | ✅ | `docs/img/capture/03-about-remoteui.png` — інтерфейс EdgeTX, зібраний із пакетів `TILE` |
| Ознака пропатченої збірки | ✅ | на тому самому знімку `VERS: pre-2.12.2-remoteui (3673c518)` — рішення задачі 0004 працює |
| Заміри стиснення | ✅ | 2.74× на фотографічній темі до 14.9× на однотонному; три плитки пішли сирими — правило «сире, якщо стиснуте більше» спрацювало на живому, а не в тесті |
| Дисципліна рецензування | ✅ | `code-reviewer` перед обома комітами; перша рецензія — 17 знахідок із блокером, друга перевіряла правки й знайшла три нові дефекти, внесені самими правками |

### Про блокер, який знайшла внутрішня рецензія

Тіньовий кадр лежав у `.bss` — 261 КБ при 192 КБ ОЗП у F429. У симуляторі це
не виявилося б **ніколи**: там пам'яті скільки завгодно. Прошивка для пульта
просто не злінкувалася б, і виявилось би це на кроці 2.4, через тижні, коли
причину шукати вже дорого.

Це і є та відповідь на питання «навіщо обов'язковий `code-reviewer`, якщо
тести зелені». Тести перевіряють поведінку. Вони не перевіряють, чи влізе це
в залізо, якого зараз немає під рукою.

Друга рецензія, яка знайшла **три нові дефекти у щойно внесених правках**, —
не менш цінна. Правки за рецензією небезпечніші за початковий код: їх пишуть
швидко, під враженням «тут же дрібниця».

### Виправлено в моїх документах за знахідками команди

1. **`docs/05-hooks.md`, пункт 5 — прибрано специфіку TX16S.** У таблиці
   стояло, що пікселі приходять «щільно, крок рядка = ширина». Це правда лише
   для непрямого режиму, тобто лише для нашого пульта. На цілях із прямим
   режимом (T18, V16, X12S) пікселі лежать у повноекранному буфері з кроком
   `LCD_W`, і чинний код прочитав би там сміття. Тобто в документ, який
   забороняє специфіку пульта, ця специфіка й пролізла.

   Рішення записане: гачок отримає **додатковий параметр — крок рядка**,
   обчислений на місці виклику. Реалізація — у задачі кроку 1.7. Окремо
   вписав застереження: перед реалізацією **перевірити за кодом LVGL**, куди
   вказує `color_p` у прямому режимі, а не припустити.

2. **Сигнатуру гачка в документі приведено до реальної.** Я писав
   `remoteUiOnFlush(area, color_p, ...)` — це затягнуло б типи LVGL у наш
   заголовок. Команда розібрала їх на місці виклику й передає цілі числа.
   Так краще, ніж я задумав: уся залежність від LVGL живе в тих 14 рядках
   патчу, які й так переглядатимуться при кожному оновленні EdgeTX.

3. **`docs/03-protocol.md` — оцінку трафіку виправлено за замірами.** Стояло
   3–8 КБ на дію в меню, виміряно 10–16 КБ. Занижено приблизно вдвічі.

4. Рішення про механізм вбудовування, сигнатуру, крок рядка й нову оцінку
   трафіку — чотири рядки в `state/DECISIONS.md`.

### Що варто відзначити окремо

Скрипт `patch-update.sh` не був у критеріях — команда додала його, зрозумівши,
що гачок зручно правити прямо в дереві EdgeTX, а без способу «зняти правку
назад» вона зникала б при першому ж `revert`. І `patch-revert.sh` **відмовляється**
працювати, якщо дерево змінене не так, як описує патч. Це саме та обережність,
якої вимагає робота з чужим кодом: не «зазвичай спрацює», а «або збігається,
або не роблю нічого».

**Вердикт: прийнято.** Усі критерії закриті доказами, які рецензент відтворив
самостійно — включно з повним циклом зняття й накладання патчу і власним
прогоном усіх 42 тестів. Чужого коду 14 рядків, усі в дозволених місцях, усі
під прапорцем. Розбіжностей між звітом і репозиторієм не знайдено.

**Що піде далі:**
1. Крок 1.5 — тестовий клієнт із вікном замість PNG.
2. У задачу кроку 1.7 — параметр кроку рядка й перевірка `color_p` у прямому
   режимі за кодом LVGL.
