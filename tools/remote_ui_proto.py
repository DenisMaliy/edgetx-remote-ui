...
# Скільки чекати доїзду решти плиток при dirtyTiles > 0.
#
# Дзеркало `webui/proto.js`; обґрунтування чисел — там же і в
# `docs/03-protocol.md`. Коротко: 3 мс — одна плитка наскрізно в найгіршому
# заміряному стані каналу (400 Б при 123 КБ/с), 50 мс — те, що від кількості
# плиток не залежить (пачка в польоті, прохід транспорту, пачка моста, Wi-Fi),
# 250 мс — стеля, ≈2.5 повних кадри дроту.
#
# ⚠️ Стеля рахується від **початку поточної низки очікування**, а не від
# останнього показаного кадру: інакше після паузи на нерухомому екрані запас
# був би вже вичерпаний, і перше ж гортання показалось би без очікування.
FRAME_WAIT_BASE_MS = 50
FRAME_WAIT_PER_TILE_MS = 3
FRAME_WAIT_MAX_MS = 250


def frame_wait_ms(dirty_tiles: int) -> int:
    """Строк очікування доїзду решти плиток, мілісекунди."""
    return min(FRAME_WAIT_MAX_MS,
               FRAME_WAIT_BASE_MS + FRAME_WAIT_PER_TILE_MS * dirty_tiles)


# --- Запобіжник: коли чекати вже немає сенсу --------------------------------
#
# Дзеркало `webui/proto.js`; повне виведення — там. Коротко: поріг відповідає
# на питання «чи встигне залишок доїхати раніше, ніж пульт перемалює кадр
# наново». Плитка ≈286 Б, дріт на 2 625 000 дає 262 500 Б/с (1.1 мс на
# плитку), кадр пульта 50 мс → 50 / 1.1 ≈ 45 плиток.
#
# ⚠️ Тут 1.1 мс на плитку, а в строку очікування — 3 мс, і це не суперечність.
# Різниць насправді дві, обидві навмисні:
#
#     строк:  400 Б (найгірша заміряна плитка) / 123 000 Б/с (Wi-Fi) = 3.25 мс
#     поріг:  286 Б (середня плитка)           / 262 500 Б/с (дріт)  = 1.09 мс
#
# Дріт — бо поріг **порівнює** доїзд залишку з доїздом наступного кадру, а
# конвеєр затримує обидва однаково й у порівнянні скорочується. Середня плитка
# — бо поріг описує типовий стан, тоді як строк описує ризик. Не зводьте ці
# числа до одного: вийде поріг 32 замість 45.

# ⚠️ Заміряно на TX16S: середня плитка 32×32 RGB565 після RLE. На ЧБ-пульті
# (етап 5.3) плитка буде в рази менша — число доведеться переміряти.
FRAME_WAIT_TILE_BYTES = 286

# Кадр пульта. ⚠️ Число обрала людина, і поруч є два сусідні, які спокушають
# «виправити» його: LVGL перемальовує раз на 30 мс (`lv_conf.h`), але це період
# перемальовки, а не період закриття нашого FRAME_END; заміряна частота
# FRAME_END у задачі 0018 — 17.1/с, тобто 58 мс. 50 мс — округлення заміряних
# 58 **вниз**, тобто в бік меншого порога: помилитись у бік «чекав дарма»
# дорожче, ніж у бік «показав одразу». 30 мс дали б поріг 27, 58 мс — 53.
RADIO_FRAME_MS = 50

# Запасне значення, коли швидкість не застосовна. ⚠️ Не стеля: на швидкості
# вищій за 2 625 000 обчислений поріг буде більший за це число.
FRAME_WAIT_TILE_LIMIT_FALLBACK = 45


def frame_wait_tile_limit(baud: int | None) -> int:
    """Скільки плиток ще має сенс чекати при цій швидкості каналу.

    Рахується зі швидкості, а не береться сталою: у цьому проєкті швидкість
    міняється на ходу (задача 0017), і поріг, прибитий до 2 625 000, на 921600
    мовчки завищив би очікування втричі.

    `baud` None або 0 — поняття не застосовне (TCP, USB CDC), тоді запасне
    значення.
    """
    if not baud:
        return FRAME_WAIT_TILE_LIMIT_FALLBACK
    bytes_per_frame = (baud / 10) * (RADIO_FRAME_MS / 1000)
    # Округлення **вниз**: питання буквально «скільки цілих плиток устигає
    # вміститися в кадр пульта». Половина плитки не доїде.
    return max(1, int(bytes_per_frame // FRAME_WAIT_TILE_BYTES))


def parse_frame_end(payload: bytes) -> int | None:
    """FRAME_END: скільки плиток лишилось незасланими на мить закриття кадру.

    Повертає None, якщо ознаки в пакеті немає, — так шле прошивка до задачі
    0019, і порожній FRAME_END лишається легальним назавжди. None означає «не
    знаю», а не «нуль»: сплутати їх — це показати зшитий кадр як цілісний.

    Довший хвіст ігнорується — правило сумісності протоколу.
    """
    if len(payload) < 2:
        return None
