...
**Етап:** 2.7. Крок 2.6 **закрито стендом і прийнято**: фази 0–4 пройдені
**Статус:** задачі 0013 і 0014 **нові**, чекають на сесію команди
**Оновлено:** 2026-08-01

## Фокус зараз

# 🎉 Пульт видно на телефоні. Дроту більше немає.

Задача [0012](../tasks/0012-bringup-bench.md) **прийнята рецензією**: стенд
піднято під наглядом людини, **ворота A і B пройдені повністю**, з воріт C
знято п'ять критеріїв із шести.

### Наступний крок — дві нові задачі

- [0013](../tasks/0013-bench-debts.md) — три борги стенда: лічильник
  розрізнення розривів на мості («сеанс закрив я» проти «сеанс помер піді
  мною»), перевірка дійсності прогону в `release_watch.py` (це і закриє
  числа C2), проба «телефон у кишені».
- [0014](../tasks/0014-native-firmware-build.md) — зібрати прошивку пульта
  нативним `arm-none-eabi-gcc 16.1.0` **без docker** (docker зник разом із
  перевстановленням системи) і довести, що вона працює. ⚠️ Робити **до**
  2 Мбод, не разом: інакше при дивній поведінці на пульті без екрана
  невідомо, на кого думати — компілятор чи швидкість.
...
відтворюваність підтверджена. ⚠️ **`docker` немає — прошивку пульта зараз
зібрати нічим.** Ця задача її не чіпала, але наступна, що чіпатиме, впреться.
