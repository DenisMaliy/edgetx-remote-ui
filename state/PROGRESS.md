...
**Етап:** 2 — залізо. Крок 2.6a закрито, 2.6 написаний і відрецензований
**Статус:** задача 0011 **у роботі** — стенд не піднятий, заміри не зроблені
**Оновлено:** 2026-07-27

## Фокус зараз

# Код готовий і вичищений. Далі — тільки залізо.

Задача [0011](../tasks/0011-esp32-bridge.md): крок **2.6a закрито**, міст і
клієнт написані, пройшли `hw-integration` перед підключенням і незалежну
рецензію коду. **12 знахідок закрито**, з них п'ять таких, що замаскувалися б
під несправність заліза.

⚠️ **Наступний крок — підняти стенд за `firmware/esp32/BRINGUP.md`, з фази 1:
три дроти, живлення від USB, дроту 5 В у джгуті немає фізично.** Конденсатори
й живлення від пульта — тільки у фазі 5, після того як фаза 4 працює.

⚠️ **Нічого не заміряно.** 2 Мбод, затримка, два місця втрат, розрив Wi-Fi,
мікшер із Wi-Fi поруч, проба на чорний прямокутник із 0010 — усе чекає. І
головне: **відпускання вводу доведене лише на програмному двійнику**, а на
ESP32 інший мережевий стек і справжній розрив замість закритого сокета.

⚠️ **Деталей бракує.** Перелік у `BRINGUP.md`; без корпусу-роз'єму Dupont 1×4
