...
**Етап:** 2 — залізо. Крок 2.6a закрито, 2.6 написаний; крок 1.7 відкладено
**Статус:** задача 0011 **у роботі** — код готовий, заміри чекають на стенд
**Оновлено:** 2026-07-26

## Фокус зараз

# Міст написаний. Далі потрібне залізо.

Задача [0011](../tasks/0011-esp32-bridge.md): крок **2.6a закрито**, міст і
клієнт написані й перевірені всім, що перевіряється без заліза.

⚠️ **Що лишилось — усе на стенді, якого немає.** Заміри (2 Мбод, затримка,
втрати плиток, розрив Wi-Fi, мікшер), проба на чорний прямокутник із 0010 і
`hw-integration` перед першим підключенням. Це не «не встигли», а «нема на
чому».
...
Обидві цілі збираються без жодного попередження: **ESP32 837 КБ** (46%
вільно), **C3 870 КБ** (42%).

| Перевірка | Чим |
|---|---|
| кадрування моста, **79** перевірок | `firmware/esp32/test/test_framing.c` |
| протокол клієнта, **62** перевірки | `node webui/proto_test.js` |
...
**Головна вимога безпеки перевірена обома способами:** і розрив сокета, і
мовчання телефона при живому сокеті дають обнулений `INPUT_STATE`. ⚠️ Поки
що на **програмному двійнику моста**, не на ESP32.
...
**Заміри задачі 0011 — стендом.** Потрібні кабель AUX1, ESP32 DevKit v1,
живлення й конденсатори. Код готовий і чекає; людина знає.
...
- ⚠️ **Борг асистенту після кроку 2.6a:** ADR-0004 і рядок «Середовище моста:
  PlatformIO + чистий ESP-IDF» у `CLAUDE.md` розходяться з ухваленим —
  збирає `idf.py`. Суть ADR не змінилась (чистий IDF без Arduino), змінився
  збирач. Обидва файли асистентові, команда їх не чіпала. Те саме стосується
  розділу «Що обрати» в `docs/10-arch-linux.md`, який лишає питання
  відкритим, — воно закрите.
