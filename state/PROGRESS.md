...
**Статус:** задача 0013 **прийнята**; наступна — 0015 (`tcpdump` під навантаженням)
**Оновлено:** 2026-08-01

## Фокус зараз

# 🎉 Борги стенда закриті. Але причина пауз вводу — поки гіпотеза.

Задача [0013](../tasks/0013-bench-debts.md) **прийнята рецензією**: усі шість
розділів критеріїв. Прошивка пульта не змінена жодним рядком, швидкість
лишилась 921600.

## ⚠️ Головне: сторож мовчання невинний, а механізм ще не пришпилений

Сторож поводиться **правильно** — він справді не чув телефона понад 750 мс.
Але не тому, що телефон мовчав. 10 хвилин під аналізатором спектра, канал
90.5%, рівно один клієнт:

| Питання | Число |
|---|---|
| Скільки разів спрацював сторож | **8 за 10 хвилин**, найдовша пауза **4196 мс** |
| Чи клієнт справді мовчав | **ні**: 2359 пакетів, його власний максимум **301 мс** |
| Чи голодувала задача моста, що чує ввід | **ні**: биття **132 мс** проти 129 у спокої |
| Затримка `ping` у тому самому вікні | максимум **356 мс** із 1142 відгуків |

⚠️ **Пояснення «ввід чекає позаду пікселів в одній трубі TCP» рецензія
відхилила, і по суті.** Напрямки TCP незалежні: пікселі йдуть міст → клієнт,
ввід — клієнт → міст, і насичення одного не ставить у чергу інший. А 356 мс на
`ping` у локальній мережі — це не «ефір вільний», а **стократне роздування**;
у таблиці це прочиталось як «ні» лише тому, що 356 < 750.

**Робоча версія тепер інша:** втрата поодинокого висхідного пакета → повторна
передача TCP з тайм-аутом від уже роздутого RTT → секунди тиші → прибуття
пачкою. Вона пояснює й «пакети не губились»: після успішної повторної передачі
втрата **не видна** на рівні застосунку взагалі.

**Наступний крок — задача [0015](../tasks/0015-tcpdump-input-latency.md):**
`tcpdump` на клієнті у вікні навантаження. Одне вимірювання, вирішальне. Є повторні передачі → розв'язок **дейтаграмний
канал** для вводу (`INPUT_STATE` є рівнем і повторюється раз на 250 мс, тобто
втрата самолікується, а повторна передача йому не потрібна). Немає → шукати в
мості. Робити **до** зміни протоколу і **до** 2 Мбод: 2 Мбод сховають симптом.

⚠️ Шкода реальна, але не та, якої боялись: клавіша **розтискається** під
пальцем, а не залипає. Безпечний бік механізму цілий.

## Телефон із заблокованим екраном втрачає сокет за хвилину

Передумова рішення від 2026-07-27 «сторож сокет не рве» **скасована заміром**:
45 с ввід ішов рівно, потім сокет помер, і за наступні 4.5 хвилини
перепідключення не було жодного. Берегти нема чого; рвати теж не починаємо.
Наслідок переїхав у клієнт: **сам перепідключається після пробудження і просить
`REFRESH`** — вимога до етапу 3.

## Розрив тепер називає причину

`/api/stats` віддає `lost_by`: `evicted`, `send_fail`, `oversized`, `ws_close`,
`socket`, `socket_silent`, `wifi`. Сума дорівнює `clients_lost`. «Помер піді
мною» дає `socket_silent +1` при **нульових** `ws.errors` і `send_dropped` — це
той самий випадок із 0012, що не мав жодної ознаки.

## Числа C2 — борг 0012 закрито

| Спосіб | Дійсних | Медіана | Розкид |
|---|---|---|---|
| сокет закрито | 6 з 9 | **90 мс** | 87…95 |
| мовчання при живому сокеті | 8 з 9 | **677 мс** | 560…747 |

Обидва вкладаються в 1000 мс тайм-ауту прошивки — міст встигає першим.

⚠️ **Перший набір цих чисел був хибний, і знайшла це рецензія коду.** Перевірка
дійсності була односторонньою: тример міг доїхати до упору **всередині** вікна
заміру, і число виходило меншим за справжнє, але проходило перевірки — так
проскочило 15 мс при медіані 89. Після другої половини перевірки розкид
стиснувся з **15…99 до 87…95 мс**.

⚠️ **Тример розганяється повільно: тримати менше за 4 с безглуздо.** Утримання
2 с давало **нуль** подій руху на всіх 12 напрямках, 12 с — від 60 до 415.
Інструмент через це виглядав зламаним («тример не рухає каналів»).
...
корпуса-роз'єма Dupont 1×4 фізично немає — див. «Заблоковано».
...
⚠️ Документ уже виправлений (`docs/09-bom.md`, 26 липня) — стримує саме
купівля, і задачею її не замінити. Це дія людини.
...
- ✅ **Борг `docs/09-bom.md` погашено 26 липня** (перевірено рецензією 0013):
  конденсатор 16 В замість 6.3, керамічний 10 мкФ середньої ланки і
  корпус-роз'єм Dupont 1×4 у документі стоять. Лишилось купити — див.
  «Заблоковано».
