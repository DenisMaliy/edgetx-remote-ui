...
**Статус:** задача [0007](../tasks/0007-input-emulation.md) виконана, на рецензії
**Оновлено:** 2026-07-26

## Фокус зараз

**Пультом можна керувати з клієнта повністю.** Задача
[0007](../tasks/0007-input-emulation.md) виконана й чекає рецензії: клавіші,
тримери, енкодер і сенсор підмішуються в **драйвери** EdgeTX, тому довге
натискання, автоповтор і прискорення дає сам EdgeTX — ми їх не програмували.
...
## Відкриті питання до асистента (із задачі 0007)

1. ⚠️ **Гарантія відпускання має межу.** Тайм-аут ловить тишу **всього
   каналу**, але не ловить втрачений поодинокий пакет «відпущено» при живому
   `PING`. Закрити може лише **періодичний повтор повного стану вводу** з боку
   клієнта — це зміна протоколу.
2. ⚠️ **`PING` раз на 2 с у `docs/03-protocol.md` тепер небезпечний**: прошивка
   на цій періодичності тримає ввід, і при 2 с клавіша розтискатиметься під
   пальцем. Наш клієнт переведено на 250 мс; специфікацію треба привести у
   відповідність, поки мостом і браузерним клієнтом ніхто не зайнявся.
3. **Прискорення енкодера** на віддаленому вводі застигає на 50 (при фізичному
   0…10), бо EdgeTX рахує його з `rotencDt` — позначки фізичного переривання,
   якої наш шлях не рухає. Наслідок: із другого клацання підряд крок у
   числових полях +25. Найдешевше — один рядок у тому самому гачку 7.
4. **Імена в `docs/05-hooks.md` розійшлися з кодом**: `remoteUiGetEncoderDelta`
   → `remoteUiGetEncoderOffset`, `remoteUiPopTouch(data)` →
   `remoteUiPopTouch(int16_t*, int16_t*, bool*)`. Обидва відхилення свідомі й
   обґрунтовані в результаті задачі; перелік закритий, тож поправити має
   асистент.
