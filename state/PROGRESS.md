...
**Статус:** задача 0013 виконана, чекає на рецензію асистента
**Оновлено:** 2026-08-01

## Фокус зараз

# 🎉 Борги стенда закриті. І сторож мовчання виявився невинним.

Задача [0013](../tasks/0013-bench-debts.md) виконана — усі шість розділів
критеріїв. Прошивка пульта не змінена жодним рядком, швидкість лишилась 921600.

## ⚠️ Головне: причина «хибного» спрацювання сторожа названа числом

Її шукали не там, і я теж почав не з того. **Сторож поводиться правильно** — він
справді не чув телефона понад 750 мс. Але не тому, що телефон мовчав.

10 хвилин під аналізатором спектра, канал 90.5%, рівно один клієнт:
...
| Чи затримка в ефірі | **ні**: `ping` 1142 відгуки, максимум **356 мс** |
| Чи губились пакети | **ні**: лічильники обох боків збігаються |

Клієнт слав вчасно, ефір був вільний, процесор задача отримувала — а пакети
спізнювались на секунди й приїжджали пачкою. **Ввід і пікселі їдуть однією
трубою TCP**, насичений потік пікселів роздуває RTT і час повторної передачі, і
крихітний пакет вводу чекає позаду.

Виправляти нема чого: поріг 750 мс мусить лишатись нижчим за 1000 мс тайм-ауту
прошивки, а вимикати сторож на завантаженому каналі — це прибрати його рівно
там, де він потрібен. **Справжній розв'язок архітектурний — окремий канал для
вводу, і це рішення асистента.** Ознака для розпізнавання — у `BRINGUP.md`,
розділ «2-біс».

⚠️ Шкода реальна, але не та, якої боялись: клавіша **розтискається** під
пальцем, а не залипає. Безпечний бік механізму цілий.

⚠️ **Пастка методики, варта запам'ятовування.** Порівняння прогонів «до/після»
тут не працює: два прогони з **однаковою** збіркою дали 3 і 27 спрацювань.
Розкид середовища вдев'ятеро перекриває будь-який ефект. Довелось міряти не
різницю прогонів, а сам механізм — прямо.

## Телефон із заблокованим екраном втрачає сокет за хвилину

⚠️ Передумова рішення від 2026-07-27 «сторож сокет не рве» **не виконується**.
Воно ухвалювалось саме заради згаслого екрана — щоб не платити перепідключенням.
Заміряно: вкладка відкрита, телефон у кишені; 45 с ввід ішов рівно, потім сокет
помер, і за наступні **4.5 хвилини** перепідключення не було жодного. Берегти
нема чого. Рішення не скасовано — воно за асистентом.

## Розрив тепер називає причину

`/api/stats` віддає `lost_by`: `evicted`, `send_fail`, `oversized`, `ws_close`,
`socket`, `socket_silent`, `wifi`. Сума дорівнює `clients_lost`. Обидва випадки
відтворені навмисно; «помер піді мною» дає `socket_silent +1` при **нульових**
`ws.errors` і `send_dropped` — це той самий випадок із 0012, що не мав ознак.
...
⚠️ **Перший набір цих чисел був хибний, і знайшла це рецензія.** Перевірка
дійсності була односторонньою: тример міг доїхати до упору **всередині** вікна
заміру, і тоді число виходило меншим за справжнє, але проходило перевірки — так
...
2 с давало **нуль** подій руху на всіх 12 напрямках, 12 с — від 60 до 415. Через
це інструмент виглядав зламаним («тример не рухає каналів»), і це коштувало
окремого розслідування з участю людини.
