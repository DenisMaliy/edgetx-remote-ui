...
**Етап:** 1 — розробка в симуляторі, крок 1.2 зроблено
**Статус:** задача 0003 виконана, чекає на рецензію; далі крок 1.3
**Оновлено:** 2026-07-25

## Фокус зараз

**Інструмент є.** Симулятор EdgeTX зібрано і перевірено: версія
...
`-DDISABLE_COMPANION=ON`. З порожнього каталогу — 134 с на 16 ядрах.

Наступний крок — **крок 1.3 плану**: каркас `remote_ui/` (протокол, CRC,
кадрування) з юніт-тестами на кадрування.
...
**Одна команда з правами root.** Симулятору для збірки бракує пакета
`python-lz4`. У сесії `sudo` просить пароль, а терміналу для нього немає,
тож збірку зроблено з віртуального середовища — обхід робочий, але
тимчасовий:
...
Після цього з команди конфігурації прибирається `-DPython3_EXECUTABLE`.
Деталі — `docs/11-simulator.md`, розділ «Залежності».

Решта стартових питань закрита 2026-07-25.
