...
```cpp
#if defined(REMOTE_UI)
  remoteUiOnFlush(area, color_p, lv_disp_flush_is_last(disp_drv));
#endif
```
...
| Параметр | Тип | Що це |
|---|---|---|
| `area` | `lv_area_t*` | межі зміненої області, логічні координати |
| `color_p` | `lv_color_t*` | пікселі цієї області, щільно, крок рядка = ширина |
| остання | `bool` | ознака кінця кадру → час слати `FRAME_END` |
