...
size_t bridge_stats_json(char *out, size_t cap)
{
    ...
    const int n = snprintf(
        ...
        "\"crc_errors\":%u,\"oversized\":%u,\"dropped\":%u},"
        ...
        U(s->ws_bytes), U(s->ws_chunks), U(s->ws_errors), U(s->tiles_dropped),
        U(s->packets_dropped), U(s->chunks_dropped), U(s->chunks_noclient), U(s->client_bytes),
        U(s->client_packets), U(s->client_input), U(s->clients_seen), U(s->clients_lost),
        U(s->input_releases), U(s->silence_timeout));
